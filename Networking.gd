...
var notifCenter = notif.instantiate()

func _ready():
	connect("packet_found",Callable(self,"_on_packet_found"))
	tcp.connect_to_host("www.mocra.eu", 40000)
	var cert = TLSOptions.client(load("res://ssl/Mocra.crt"), "www.mocra.eu")
	print("STATUS = ", con.connect_to_stream(tcp, "www.mocra.eu", cert))
	check_timer.connect("timeout",Callable(self,"check_queue"))
	check_timer.set_wait_time(0.05)
	check_timer.set_one_shot(false)
	get_node(".").add_child(check_timer)
	check_timer.start()
	auto_packet_checker.connect("timeout",Callable(self,"_auto_packet_checker"))
	get_node(".").add_child(auto_packet_checker)
	auto_packet_checker.set_wait_time(10)
	auto_packet_checker.start()
	animation_init()
	send_data("get_server_infos")


func send_data(data:String):
	con.put_data(data.to_utf8_buffer())
...
func check_queue():
	if !waiting_queue.is_empty() and current == null:
		...
		con.put_data(head[0].to_utf8_buffer())
		current = head
